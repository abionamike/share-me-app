...
const PinDetail = ({ user }: { user: User | null }) => {
  ...
  const [pinDetail, setPinDetail] = useState<PinInterface | null>(null);
    ...
    <>
      <div className="flex xl:flex-row flex-col m-auto bg-white" style={{ maxWidth: '1500px', borderRadius: '32px' }}>
        <div className="flex justify-center items-center md:items-start flex-initial">
          <img src={pinDetail.image && urlFor(pinDetail.image.asset.url).url()} className="rounded-t-3xl rounded-b-lg" alt="pin detail" />
        </div>
        ...
        <div className="w-full p-5 flex-1 xl:min-w-620">
          <div className="flex items-center justify-between">
            ...
            <a onClick={(e) => e.stopPropagation()} href={pinDetail.destination} target="_blank" rel="noopener noreferrer">{pinDetail.destination}</a>
          </div>
          ...
          <div className="flex flex-wrap mt-6 gap-3">
            <Link to={`/user-profile/${pinDetail.postedBy?._id}`}>
              <img src={pinDetail.postedBy?.image} className="w-8 h-8 rounded-full cursor-pointer" alt="user-profile" />
            </Link>
            <input 
              type="text" 
